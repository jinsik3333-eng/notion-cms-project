...
- **Claude API (Anthropic)** - Structured Output 방식 5가지 관점 분석
...
- **Task 003**: 기본 레이아웃 및 Header/Footer 컴포넌트 - **우선순위**
  - 예상 소요 시간: 3~4시간
...
### Phase 3: 핵심 기능 개발 (Claude API 분석)

> ResumeLens의 핵심 가치인 AI 분석 기능을 구현합니다.
> Claude API 연동과 분석 결과 표시 UI가 완성되어야 서비스의 핵심 플로우가 동작합니다.

- **Task 007**: Claude API 연동 - POST /api/analyze-resume (F002)
  - 예상 소요 시간: 4~6시간
  - 완료 기준: Postman 또는 curl로 `/api/analyze-resume`에 자소서 텍스트를 POST하면 5가지 분석 결과가 담긴 JSON을 반환함
  - 세부 구현사항:
    - [ ] `app/api/analyze-resume/route.ts` - Next.js Route Handler 생성
    - [ ] `@anthropic-ai/sdk` 패키지 설치 및 Claude API 클라이언트 초기화
    - [ ] System Prompt 작성: 5가지 관점 동시 분석 지시 (한국어)
    - [ ] Structured Output 스키마 정의 (AnalysisResult 타입과 일치)
    - [ ] 요청 검증: resumeText 50~5,000자 범위 체크
    - [ ] 에러 처리:
      - 429 Rate Limited: retryAfter 초 반환
      - 500+: 사용자 친화적 에러 메시지
      - 60초 타임아웃 처리
    - [ ] 환경 변수 `ANTHROPIC_API_KEY` 설정 (`.env.local`)
  - 테스트 체크리스트:
    ...
    - [ ] 클라이언트에서 ANTHROPIC_API_KEY가 노출되지 않는지 확인
...
> 핵심 분석 기능(Phase 3)이 완성된 후 작업하여 MVP 우선순위를 지킵니다.
...
- **Task 016**: 에러 핸들링 및 엣지 케이스 처리
  - 예상 소요 시간: 2~3시간
  - 완료 기준: Claude API 오류, Notion API 오류, 잘못된 URL 직접 접근 등 주요 엣지 케이스에서 앱이 크래시 없이 적절한 UI를 표시함
  - 세부 구현사항:
    - [ ] `app/not-found.tsx` - 404 페이지 완성
    - [ ] `app/error.tsx` - 전역 에러 바운더리 추가
    - [ ] Claude API 429 에러 시 "30초 후 자동 재시도" 안내 UI
...
| Phase 2: 공통 컴포넌트 완성 | D+2 | 대기 | Header, Footer, 입력 폼 |
| Phase 3: Claude API 분석 기능 | D+6 | 대기 | 서비스 핵심 가치 완성 |
| Phase 4: Notion CMS 연동 | D+10 | 대기 | 마케팅 페이지 완성 |
| Phase 5: 최적화 및 배포 | D+12 | 대기 | Vercel 프로덕션 배포 |
| MVP 완성 | D+14 | 대기 | 전체 플로우 테스트 완료 |
...
- **Claude API는 서버사이드 호출만**: Route Handler(`app/api/`)에서만 호출하여 API 키 노출 방지
...
- **Claude API 가용성**: 분석 기능 전체가 Anthropic API에 의존
...
| 리스크 | 영향도 | 완화 방안 |
|-------|--------|----------|
| Claude API 응답 지연 (30~60초) | 높음 | 로딩 상태 상세 메시지 표시, 60초 타임아웃 설정 |
| Claude API 429 Rate Limit | 중간 | 30초 후 자동 재시도 안내, 사용자 친화적 메시지 |
...
- [ ] 저장소 클론 후 `npm install` 실행
- [ ] `.env.local` 파일 생성 (`.env.local.example` 참고)
- [ ] Notion DB 3개(PricingPlans, Testimonials, ContentPages) 생성 및 Integration 연결
- [ ] Claude API 키 발급 (Anthropic Console)
- [ ] `npm run dev` 실행하여 로컬 개발 서버 확인
...
| 지표 | 목표 | 측정 방법 |
|------|------|----------|
| 빌드 성공 | 오류 0개 | `npm run build` 출력 |
| TypeScript 타입 오류 | 0개 | `npx tsc --noEmit` |
| ESLint 오류 | 0개 | `npm run lint` |
| Lighthouse 성능 | 80점 이상 | Chrome DevTools Lighthouse |
| Claude API 응답 | 60초 이내 | 실제 테스트 측정 |
| Notion API 캐시 히트율 | 80% 이상 | Vercel Analytics |
...
- Claude API 사용량 및 비용 모니터링 (Anthropic Console)
...
- [ ] PRD의 모든 핵심 요구사항(F001~F011)이 Task로 구현됨
- [ ] 자소서 50자 미만 / 5,000자 초과 입력 시 오류 메시지 표시 확인
- [ ] Claude API 응답이 5가지 분석 + 종합 점수 구조를 포함하는지 확인
- [ ] 분석 결과 색상 코딩 (0-59 빨강, 60-79 노랑, 80+ 초록) 확인
- [ ] Notion API 3개 엔드포인트(pricing, testimonials, content) 정상 동작 확인
- [ ] 각 Notion 엔드포인트 캐시 설정(24h, 12h, 6h) 적용 확인
- [ ] 모바일 반응형 레이아웃 확인 (Chrome DevTools 모바일 에뮬레이션)
- [ ] `.env.local.example` 파일에 모든 필요 환경 변수 명시됨
- [ ] `npm run build` 오류 없음
- [ ] `npm run lint` 오류 없음
- [ ] Vercel 배포 후 프로덕션 URL에서 전체 플로우 테스트 완료
